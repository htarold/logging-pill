...
int8_t bosch_init(struct bosch * bp, uint8_t bosch_addr)
{
  ...
  /* Get chip ID first */
  reg = 0xd0;
  er = wi2c_transfer7(PARAMS(bp), &reg, 1, &(bp->chip_id), 1);
  dbg(tx_msg("wi2c_transfer7 returned ", er));
  if (er) return(er);
  ...
  if (BOSCH_IS_BMP(bp))
    ...
    count = (0xa1 - 0x88)+1;
  reg = 0x88;
  er = wi2c_transfer7(PARAMS(bp), &reg, 1, (void *)&(bp->T1), count);
  ...
  count = (0xe7 - 0xe1)+1;
  reg = 0xe1;
  er = wi2c_transfer7(PARAMS(bp), &reg, 1, (void *)&(bp->H2), count);
  dbg(tx_msg("i2c_transfer7 returned ", er));
  if (er) return(er);
...
int8_t bosch_read_start(struct bosch * bp)
{
  ...
  regs[0] = 0xe0;                     /* software reset register */
  regs[1] = 0xb6;                     /* reset code */
  er = wi2c_transfer7(PARAMS(bp), regs, 2, 0, 0);
  dbg(tx_msg("Software reset result = ", er));
  if (er) return er;
  ...
  if (BOSCH_IS_BME(bp)) {
    ...
    er = wi2c_transfer7(PARAMS(bp), regs, 2, 0, 0);
    dbg(tx_msg("BME humidity oversample wi2c_write returned ", er));
    if (er) return(er);
  }
  ...
  regs[2]                             /* config register */
	  ...
	  | 0x0;                      /* SPI unused */
  er = wi2c_transfer7(PARAMS(bp), regs, 3, 0, 0);
  dbg(tx_msg("BME ctrl/meas wi2c_write returned ", er));
  if (er) return(er);
...
int8_t bosch_read_end(struct bosch * bp)
{
  ...
  er = wi2c_transfer7(PARAMS(bp), &reg, 1, ary, count);
  dbg(tx_msg("read_end wi2c_transfer7 returned ", er));
  if (er) return(er);
...
int8_t bosch_is_busy(struct bosch * bp)
{
  ...
  er = wi2c_transfer7(PARAMS(bp), &reg, 1, &busy, 1);
  if (er) return(er);
  busy &= ((1<<3) | (1));
